  ...
  const fetchHeroData = async () => {
    try {
      ...
      if (docSnap.exists()) {
        ...
        setTitle(data.title || "");
        setPromotionBadge(data.promotionBadge || "");
        ...
        <CardContent className="p-6 sm:p-8">
          <form onSubmit={handleSave} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-[#faf9f5] p-6 rounded-2xl border border-gray-100">
              <div className="space-y-3">
                <Label htmlFor="promotionBadge" className="text-base font-bold text-gray-700">Promotion Badge</Label>
                <Input
                  id="promotionBadge"
                ...
                />
                <p className="text-xs text-gray-500 font-medium mt-1">Shows up as a glowing pill above the title.</p>
              </div>
              <div className="space-y-3">
                <Label htmlFor="subtitle" className="text-base font-bold text-gray-700">Subtitle</Label>
                <Input
                  id="subtitle"
                  value={subtitle}
                  onChange={(e) => setSubtitle(e.target.value)}
                  placeholder="Discover the rich, aromatic flavors..."
                  className="h-12 rounded-xl bg-white border-gray-200 focus-visible:ring-[#e87a1e] focus-visible:border-[#e87a1e] transition-colors"
                />
                <p className="text-xs text-gray-500 font-medium mt-1">Secondary text displayed below the main title.</p>
              </div>
            </div>
